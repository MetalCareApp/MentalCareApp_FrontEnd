...
import { PSS_TEST_QUESTION_DATA } from '../../utils/TestData';
import PSSTestQuestionCard from '../../components/PSSTestQuestionCard';
import { useState } from 'react';
import Toast from 'react-native-toast-message';
import { useNavigation } from '@react-navigation/native';
import { createPSSExamination } from '../../apis/examinationApi';
import CustomText from '../../components/common/CustomText';

function PSSTestScreen() {
  ...
  const onSubmit = async () => {
    if (scores.length < PSS_TEST_QUESTION_DATA.length) {
      Toast.show({
        type: 'error',
        text1: '모든 질문에 답변해주세요.',
        position: 'bottom',
    ...
    await createPSSExamination(scoreList);
    // @ts-ignore
    navigation.navigate('pss_test_result', { totalScore });
  };
